        ...
        <AnimatePresence>
            {ModalComponent && resolver && (
                <div className="top-0 absolute w-screen h-screen flex content-center items-center justify-center z-[90]">
                    <motion.span key="modal-overlay" onClick={() => modalSevice.resolve({ exitCode: ModalExitCode.NO_CHOICE })} className="absolute top-0 bottom-0 right-0 left-0 bg-black" initial={{ opacity: 0 }} animate={{ opacity: ModalComponent && 0.6 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }} />
