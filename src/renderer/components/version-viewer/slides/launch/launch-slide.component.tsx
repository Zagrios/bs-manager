...
export function LaunchSlide({ version }: Props) {
    ...
    const toggleActiveLaunchMod = (checked: boolean, launchMod: LaunchMod | string) => checked
        ? setActiveLaunchMods(prev => [...prev, launchMod])
        : setActiveLaunchMods(prev => prev.filter(mod => mod !== launchMod));

    const togglePinnedLaunchMod = (pinned: boolean, launchMod: LaunchMod | string) => pinned
